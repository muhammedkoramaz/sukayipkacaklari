...
> (yalnızca ana sayfada ve site dışından gelen ilk ziyarette; `TR | EN` tıklaması
> `localStorage['le-lang']`'e yazılıp seçimi sabitler), header+footer
...
>
> **Dil değiştirici / açılış davranışı:** tek yer — `<head>` içindeki açılış script'i (`OPEN_SCRIPT`). (1) `hreflang="tr|en"` taşıyan bir `<a>`'ya tıklamayı **capture fazında** `document` üzerinde dinleyip `localStorage['le-lang']` yazar; dinleyici `<head>`'de bağlandığı için `defer`'li `site.min.js` henüz yüklenmemişken yapılan **ilk** tıklama da kaydedilir (eskiden `site.js`'teki dinleyici geç bağlandığı için ilk tıklama kaybolup kullanıcı geldiği dile geri sekiyordu). (2) Kayıtlı tercih varsa tüm sitede o dile sabitler. (3) Tercih yoksa tarayıcı dili tespiti **yalnızca** `data-home="1"` sayfada (`/`, `/en/`), **site dışından** gelen ziyarette (aynı origin `document.referrer` varsa çalışmaz) ve `sessionStorage['le-lang-redirected']` ile **oturum başına bir kez** çalışır — site içi gezinme asla ezilmez. `.nav__lang` stili `site.css` sonunda.
...
| **F** (2026-09-03) | **Dil değiştirici ilk tıklama hatası** — TR ana sayfada `TR | EN`'e yapılan ilk tıklama işlemiyor, kullanıcı ikinci tıklamada dil değiştirebiliyordu. Sebep: seçimi kaydeden dinleyici `defer`'li `site.min.js` içindeydi; henüz bağlanmamışken yapılan tıklamada tercih yazılmıyor, hedef sayfadaki açılış script'i de tarayıcı diline bakıp kullanıcıyı geri gönderiyordu (ikinci tıklama yalnızca `le-lang-redirected` oturum bayrağı sayesinde tutuyordu). Çözüm: tıklama yakalama (capture) dinleyicisi `<head>` açılış script'ine taşındı · tarayıcı dili tespiti artık site dışından gelen ziyaretle sınırlı (aynı origin `document.referrer` varsa çalışmaz, yani `localStorage` tamamen kapalıysa da doğru çalışır) · oturum bayrağı yalnızca tespit dalını kilitler, açık seçime dokunmaz · `site.js`'teki eski dinleyici kaldırıldı (54 HTML + 2 jeneratör + `site.min.js`). Doğrulama: 54 HTML 0 sorun · Chromium ile 18 senaryo (yarış durumu, `localStorage` kapalı, ileri-geri tıklama) |
| **G** (2026-09-04) | /rehber/ → /blog/ taşındı (301'li), hub+makale kartları görselli, 8 yeni makale (debi ölçümü, basınç yönetimi, adım testi, sıfır basınç testi, hidrolik modelleme, boru hattı tespiti, şebeke haritalama/CBS, kaçak onarımı) TR+EN |
